...
for fname in filenames:
    data = np.array(np.loadtxt(fname, dtype=float))
    if (fname.find('large') != -1):
        if (fname.find('eval') != -1):
            if (fname.find('reward') != -1):
              ...
              IMPlarge_rate_eval = np.append(IMPlarge_rate_eval,data)
        ...
        if (fname.find('eval') != -1):
            ...
            if (fname.find('reward') != -1):
                ...
                IMPshort_rate_test = np.append(IMPshort_rate_test,data)


print((IMPlarge_rate_eval))
print(type(data))
